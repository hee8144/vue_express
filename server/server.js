...
app.get('/prof/list', async (req, res) => {
  ...
  try {
    ...
    const count = await connection.execute(`SELECT COUNT(*) FROM PROFESSOR `);
    res.json({
        result : "success",
        profList : rows,
...
app.get('/board/list', async (req, res) => {
  ...
  try {
    const result = await connection.execute(
      `SELECT B.*, TO_CHAR(CDATETIME, 'YYYY-MM-DD') AS CDATE FROM TBL_BOARD B OFFSET ${offset} ROWS FETCH NEXT ${pageSize} ROWS ONLY `
    );
    const columnNames = result.metaData.map(column => column.name);
    // 쿼리 결과를 JSON 형태로 변환
    const rows = result.rows.map(row => {
